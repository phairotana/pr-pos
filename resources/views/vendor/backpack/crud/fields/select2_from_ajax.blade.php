...
@php
    ...
    $field['notify-error'] = $field['notify-error'] ?? '';
@endphp
...
<script>
    function bpFieldInitSelect2FromAjaxElement(element) {
        ...
        var $funcitonErrorDisplay = element.data('error-calling-func')
        var notify  = "{{ $field['notify-error'] }}" ?? '';
            ...
            $selectedOptions != [])
        {
            ...
            select2AjaxFetchSelectedEntry(element).then(function(result) {
                /* result has pagination so that with need to point data object  */
