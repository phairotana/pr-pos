...
$field['has_own_depend'] = $field['has_own_depend'] ?? '';
$field['fetch_depend_url'] = $field['fetch_depend_url'] ?? '';
$field['flow_depend'] = $field['flow_depend'] ?? '';
$field['default_flow_depend'] = $field['default_flow_depend'] ?? '';
$field['trigger_on_change'] = $field['trigger_on_change'] ?? '';
...
@if ($crud->fieldTypeNotLoaded($field))
    ...
    @push('crud_fields_scripts')
        <script>
            var urlFetchingData = "{{ $field['fetch_depend_url'] }}";
            var flowDepend = @json($field['flow_depend']);
            var defaultValue = @json($field['default_flow_depend']);
            var triggerChange = "{{ $field['trigger_on_change'] }}";
            ...
            function updateFormRepeatData(res, element) {
                $parentDiv = $(element)
                var objectKey = Object.keys(flowDepend)

                objectKey.forEach(element => {
                    var selector = $parentDiv.find(`input[data-repeatable-input-name="${element}"]`)
                    /* if it has default values */
                    var apiVal = res[flowDepend[element]]
                    if(Object.keys(defaultValue).includes(element)){
                        $(selector).val(defaultValue[element])
                    }else{
                        $(selector).val(apiVal)
                    }
                });
            ...
            function bpFieldInitRepeatableElement(element) {
                /*
                    Customize field with repeatable changes
                */
                $(document).ready(function() {
                    var $attrProduct = '.product_id_select';

                    $('body').on('change', $attrProduct, function() {

                        var self = this
                        var $wrapperSelf = $(self).parent().parent()
                        var val = $(this).val()
                        if (val !== null) {
                            axios.get(urlFetchingData + "/" + val).then(function(res) {
                                res = res.data.data;
                                updateFormRepeatData(res, $wrapperSelf)
                                $('body').trigger(triggerChange)
                            })
                        } else {
                            updateFormRepeatData({}, $wrapperSelf)
                            /* remove default value on repeatables */
                            Object.keys(defaultValue).forEach(el=> {
                                $wrapperSelf.find(`input[data-repeatable-input-name="${el}"]`).val('')
                            })
